...
s=f'''a) F(2,3)=F(x-y,y)+1,    {mt.fun1(2,3)} 
b) F(5,6)=F(n-1,r)+F(n-1,r-1)   {mt.fun2(5,6)}
c) F(n)=F(n/2)+1 if n>1         {mt.fun3(5)}
d) F(5,6)=1 if M=0, or M ≥N ≥1, and F(M,N)=F(M-1,N)+F(M-1,N-1), otherwise.   {mt.fun4(5,6)}
e) B(5,6)=m!/(x!(m-x)!) where m>x,       
